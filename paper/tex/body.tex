...
size rather than evidence of equality. Wilcoxon on the same pairs is
\RunIdentityWilcoxP{} and the median difference \RunIdentityMedian{} has
interval \RunIdentityMedianCiLo{} to \RunIdentityMedianCiHi{}, also
including zero.
...
of \RunPromptSignP{}. Wilcoxon on those pairs returns \RunPromptWilcoxP{},
and the mean interval \RunPromptCiLo{} to \RunPromptCiHi{} excludes zero.
The protocol fixed that rank before the run and the paper keeps it: the
count is reported as the declared secondary, it is not promoted to a
confirmatory claim, and the identity conclusion rests on the primary
endpoint alone.
...
\RunClockIdentityHigher{} of the twelve clock pairs. The descriptive mean
intervals run from \RunDogIdentityCiLo{} to \RunDogIdentityCiHi{} (dog) and
from \RunClockIdentityCiLo{} to \RunClockIdentityCiHi{} (clock); they pull
in opposite directions and sit outside the confirmatory family.
...
dispersion ratios are \RunDispersionDog{} (dog; interval
\RunDispersionDogLo{} to \RunDispersionDogHi{}) and \RunDispersionClock{}
(clock; interval \RunDispersionClockLo{} to \RunDispersionClockHi{}). Both
point estimates are below one, the reverse of the historical point; both
intervals include one, and both are reported as description.
...
concepts; the dog and clock identity counts differ in sign, their mean
intervals pull in opposite directions, and both are read as description
rather than as a second test family. The registered Wilcoxon check and the
median identity interval agree with the confirmatory sign test and do not
form a second confirmatory claim.
