...
evidence-manifest binding; the figure code verifies each bound file before reading it and
stops the build on any mismatch. That half trained nothing and launched no
new evaluation. The registered half is a later, declared execution of the
protocol below, bound under evidence ID E-PREDECL.
...
from the bound analysis, so the design counts cannot be mistaken for the
...
pre-declaration (evidence ID E-PREDECL). Pairing unit, arms,
...
refuses to emit run macros unless those recorded facts match the declared
design: \RunPairs{} realised pairs, and the historical grid remaining
\NPairs{} prompts.
...
recorded on disk as not computed and are not computed here either. The historical
authorisation record remains the status at protocol-design time; the single
authorised execution is the run declared in
E-PREDECL and bound as the registered result. Re-runs and
unregistered variants are out of scope.
